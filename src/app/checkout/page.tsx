...
export default function CheckoutPage() {
    ...
    if (loading) return <div className="min-h-screen pt-32 text-center text-xl">{t('checkout.loading')}</div>;
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 min-h-screen bg-gray-50">
            <h1 className="text-4xl font-serif font-bold text-gray-900 mb-10">{t('checkout.title')}</h1>
            ...
            <div className="flex flex-col lg:flex-row gap-12">
                ...
                <div className="lg:w-2/3 space-y-8">
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                        <div className="flex items-center space-x-3 mb-6">
                            ...
                            <h2 className="text-2xl font-bold text-gray-900">{t('checkout.shippingAddress')}</h2>
                        </div>
                        ...
                        {addresses.length > 0 ? (
                            ...
                            <p className="text-gray-500 mb-6 italic">{t('checkout.noAddresses')}</p>
                        )}
                        ...
                        {!showAddressForm ? (
                            <button onClick={() => setShowAddressForm(true)} className="flex items-center text-amber-600 font-medium hover:text-amber-800 transition-colors">
                                <Plus className="w-5 h-5 mr-1" /> {t('checkout.addNewAddress')}
                            </button>
                        ) : (
                            <form onSubmit={handleAddAddress} className="mt-6 bg-gray-50 p-6 rounded-2xl border border-gray-200">
                                <h3 className="font-bold text-gray-900 mb-4">{t('checkout.newAddressDetails')}</h3>
                                ...
                                <div className="flex justify-end mt-4 space-x-3">
                                    <button type="button" onClick={() => setShowAddressForm(false)} className="px-5 py-2 text-gray-600 font-medium hover:bg-gray-200 rounded-xl transition-colors">{t('checkout.cancel')}</button>
                                    <button type="submit" className="px-5 py-2 bg-gray-900 text-white font-medium hover:bg-amber-600 rounded-xl transition-colors">{t('checkout.saveAddress')}</button>
                                </div>
                            </form>
                        )}
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                        <div className="flex items-center space-x-3 mb-6">
                            ...
                            <h2 className="text-2xl font-bold text-gray-900">{t('checkout.paymentMethod')}</h2>
                        </div>
                        <div className="p-5 rounded-2xl border-2 border-amber-600 bg-amber-50 flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-gray-600 text-sm">{t('checkout.codDesc')}</p>
                            </div>
                ...
                <div className="lg:w-1/3">
                    <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 sticky top-24">
                            ...
                            {safeCartDetails.map((item) => (
                                <div key={item.product?._id} className="flex justify-between items-center pb-4 border-b border-gray-50 last:border-0 last:pb-0">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-sm text-gray-500">{t('checkout.qty')}: {item.quantity}</p>
                                    </div>
                        ...
                        >
                            {t('checkout.placeOrder')} <ChevronRight className="w-5 h-5 ml-2" />
                        </button>
                    </div>
                </div>
